...
//  }
		$typedipliquations=$this->Typedipliquation->find('list');
                $clients = $this->Client->find('list');
                 $this->set(compact('typedipliquations','pointdeventes','exerciceid','exercices','date1','date2','clientid','clients','devis',$this->paginate()));
	...
	public function add($type=null) {
		...
		if ($this->request->is('post')) {
         ...
         array('fields' =>array('MAX(Devi.numeroconca) as num'),
          'conditions' => array('Devi.pointdevente_id'=>$pv))
         );
         //debug($numero);die;
			...
			if ($this->Devi->save($this->request->data)) {
                         $id=$this->Devi->id;
                              ...
                              if ($f['sup']!=1){
                                ...
                                $Lignedevis['totalht']=($f['prixhtva']*(1-$f['remise']*0.01))*$f['quantite'];
                                $Lignedevis['totalttc']=((($Lignedevis['totalht']))*(1+($f['tva']*0.01)));  
           ...
           $reglementlibre = $this->Reglementclient->find('all', array('fields'=>array('sum(Reglementclient.Montant) as reglibretotale')
            ...
            //fin info client************************************************
         	...
         	$depots = $this->Factureclient->Depot->find('list',array('fields' => array('Depot.designation'),'conditions'=>array('Depot.id'=>$p)));
                 ...
                 $articles=$this->Article->find('list', array( 'conditions' => array('Article.typeetatarticle_id'=>1),'recursive'=>-1)) ;
		$this->set(compact('name','autorisation','solde','tabqtestock','depots','valreste','matriculefiscale','adresse','pointdeventes','clients','mm','articles','lignedevis','date','type'));
	}
 ...
 */
  ...
  if ($anne==date("Y")){
    ...
    //**************************************trouver les qte en stock de chaque article pour chaque depot *******************************************************
		...
		$typedipliquations=$this->Typedipliquation->find('list');
                $x=$model;
                $this->set(compact('model_ans','ligne_ans','ligne','x','clientid','typedipliquations','name','autorisation','solde','tabqtestock','depots','valreste','matriculefiscale','adresse','pointdeventes','clients','mm','articles','lignedevis','date','type'));
	}
