...
foreach ($depots as $dep) {
      ...
      //debug(@$condb7);
            ...
            'conditions' => array(@$condb12,@$condb11,@$condb6,@$condb7,'Bonlivraison.factureclient_id'=>0),'recursive'=>0 ));
            //debug($soldebl);die;
            if(!empty($soldebl)){
            ...
            'conditions' => array(@$condf12,@$condf11,@$condf6,@$condf7),'recursive'=>0 ));
            if(!empty($soldebl)){
            ...
            $bonecart=$this->Bonecart->find('first', array(
            'fields'=>array('sum(Bonecart.quantite) as quantite'),    
            'conditions' => array(@$condi12,@$condi11,@$condBecr6,@$condBecr7),'recursive'=>2 ));
            if(!empty($bonecart)){
                //debug($transfert[0]['quantite']);
                $solde=$solde+$bonecart[0]['quantite'];
                //debug("transfert_entreé".$solde);
            }
            }
...
//******************************************************************************************************************************        
            ...
            'conditions' => array(@$condb1,@$condb2,@$condb3,@$condb4,@$condb6,@$condb7,'Bonlivraison.factureclient_id'=>0),'recursive'=>0 ));
            if ($livrison==0) {
...
//******************************************************************************************************************************        
            ...
            'conditions' => array(@$condf1,@$condf2,@$condf3,@$condf4,@$condf6,@$condf7),'recursive'=>0 ));
            if ($facture==0) {
            $lignefactures=array();
            } 
...
//******************************************************************************************************************************        
            ...
            $tablignereceptions['numero']=@$lignereception['Bonreception']['numerofrs'];
            $tablignereceptions['article']=$lignereception['Article']['name'];
            $tablignereceptions['qte']=$lignereception['Lignereception']['quantite'];
            $tablignereceptions['pu']=$lignereception['Lignereception']['totalht']/$lignereception['Lignereception']['quantite'];
...
//******************************************************************************************************************************        
            ...
            $tabfacturefours['numero']=@$facturefour['Facture']['numerofrs'];
            $tabfacturefours['article']=$facturefour['Article']['name'];
            $tabfacturefours['qte']=$facturefour['Lignefacture']['quantite'];
            $tabfacturefours['pu']=$facturefour['Lignefacture']['totalht']/$facturefour['Lignefacture']['quantite'];
...
//**************************************************************************************************************************       
        ...
        $this->set(compact('depots','typeligneventes','clientid','articleid','fournisseurid','date1','date2','historiquearticles','fournisseurs','clients','articles','exercices','lignedevis','lignecommandes','lignelivrisons','lignefactures','name','exerciceid'));
