    ...
    public function transformation($model = null, $ligne_model = null, $attribut = null, $liste_in = null, $model_ans = null, $ligne_model_ans = null, $attribut_ans = null) {
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if ($model_ans == 'Bonlivraison') {
				...
				//$this->request->data[$model]['Montant_Regler'] = $this->request->data['Factureclient']['Montant_Regler'];
			} else {
            ...
            $this->request->data[$model]['numeroconca'] = $mm;
            $this->request->data[$model]['numero'] = $numspecial;
            //debug($this->request->data[$model]);die();
            $this->$model->create();
            if ($this->$model->save($this->request->data)) {
                $id = $this->$model->id;
                $this->misejour($model, "add", $id);
...
//            debug($lignes);
		...
		$bonliv = $this->Bonlivraison->find('first', array(
			'conditions' => array('Bonlivraison.id' =>$liste_in), 'recursive' => -1));
	        //debug($bonliv);die();
       ...
       // debug($sumttc);die();
        ...
        $this->set(compact('avectimbre','attribut_ans', 'ligne_model_ans','bonliv', 'model_ans', 'lignes', 'entete', 'ligne_model', 'model', 'typeclient_id', 'name', 'autorisation', 'solde', 'valreste', 'matriculefiscale', 'adresse', 'pointdeventes', 'clients', 'utilisateurs', 'timbre', 'depots', 'date', 'lignefactureclients', 'articles', 'tabqtestock', 'mm', 'numspecial'));
    }
