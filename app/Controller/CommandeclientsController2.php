...
//  }   
		...
		$typedipliquations=$this->Typedipliquation->find('list');
                $clients = $this->Client->find('list');
                 $this->set(compact('typedipliquations','pointdeventes','exerciceid','exercices','date1','date2','clientid','clients','commandeclients'));
		...
		$clients = $this->Commandeclient->Client->find('list');
            ...
            //fin info client************************************************
         	...
         	$depots = $this->Factureclient->Depot->find('list',array('conditions'=>array('Depot.id'=>$p)));
                 ...
                 $articles=$this->Article->find('list', array( 'conditions' => array('Article.typeetatarticle_id'=>1),'recursive'=>-1)) ;
		$this->set(compact('typeclient_id','name','autorisation','solde','tabqtestock','articles','depots','valreste','matriculefiscale','adresse','lignelivraisons','clientid','pointdeventes','pntv','clients','mm','articles','lignelivraisons','numspecial'));
	}
/**
 * edit method
 *
 ...
 */
  ...
  //*********** zeinab *************//
        ...
        $soc= CakeSession::read('soc');
            ...
            if ((!empty($this->request->query['dateliv1']))) {
                $dateliv1 = date("Y-m-d", strtotime(str_replace('/', '-', $this->request->query['dateliv1'])));
                $conddateliv1 = 'Commandeclient.dateliv >= ' . "'" . $dateliv1 . "'";
                $cond4 = "";
            }
            ...
            if ((!empty($this->request->query['dateliv2']))) {
                $dateliv2 = date("Y-m-d", strtotime(str_replace('/', '-', $this->request->query['dateliv2'])));
                $conddateliv2 = 'Commandeclient.dateliv <= ' . "'" . $dateliv2 . "'";
                $cond4 = "";
            }
            if ($this->request->query['clientid']) {
                $clientid = $this->request->query['clientid'];
                $cond3 = 'Commandeclient.clientid =' . $clientid;
            }
