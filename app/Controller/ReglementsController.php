...
//		$familles = $this->Famille->find('list');
        ...
        if ((isset($this->request->data) && !empty($this->request->data)) || (( in_array(CakeSession::read('view'), Array("edit", "view", "delete"))) && (CakeSession::read('Controller') == "Reglements"))) {
            ...
            if (!empty($this->request->data['Recherche']['exerciceid'])) {
                $exerciceid = $this->request->data['Recherche']['exerciceid'];
                $conde = 'Reglement.exercice_id =' . $exercices[$exerciceid];
                $cond = '';
                $cond1 = '';
            }

            if ($this->request->data['Recherche']['Date_debut'] != '__/__/____' &&(!empty($this->request->data['Recherche']['Date_debut']))) {
            ...
            if ($this->request->data['Recherche']['Date_fin'] != '__/__/____' &&(!empty($this->request->data['Recherche']['Date_fin']))) {
            ...
            if ($this->request->data['Recherche']['libre_id']) {
                $libre_id=$this->request->data['Recherche']['libre_id'];
                if($libre_id=='Oui'){
                    $condlibre = 'Reglement.montantaffecte=0';
                }else{
                    $condlibre = 'Reglement.montantaffecte!=0';
                }
        ...
        $reglementfournisseurs = $this->Reglement->find('all', array('conditions' => array(@$pv, @$conde, @$cond, @$cond1, @$cond2, @$cond3, @$cond4, @$cond6, @$cond7,@$condlibre)));
...
//              $reglementfournisseurs= $this->paginate();
        ...
        $this->set(compact('exerciceid','libres','libre_id', 'exercices', 'societes', 'pointdeventes', 'pointdevente_id', 'societe_id', 'reglementfournisseurs', 'collections', 'transferecommandebls', 'marques', 'familles', 'fournisseurs', 'ligneclients', 'marque_id', 'Date_debut', 'Date_fin', 'fournisseur_id', 'num_recu'));
...
//              $this->loadModel('Famille'); 
        ...
        $this->paginate = array(
            'order' => array('Reglement.id' => 'desc'),
            'conditions' => array($cond, $cond1, $cond2, $cond3, $cond4, $cond6, $cond7,$condlibre,@$conde));
            ...
            if ($this->Reglement->save($this->request->data)) {
                ...
                if (!empty($this->request->data['Lignereglement'])) {
                    foreach ($this->request->data['Lignereglement']as $j => $l) {
                        if ((!empty($l['facture_id']))) {
                            ...
                            if ($mntt > 0 && array_key_exists('facture_id', $l)&& $l['Montant']!='') {
                                $li = array();
                                ...
                                $tc = 1;
                                if ($devisefournisseurs != 1)  $tc = $li['tauxchange'];
                                
                                $this->Facture->updateAll(array('Facture.Montant_Regler ' =>  'Facture.Montant_Regler+' . $l['Montant'], 'Facture.tauxchange ' => $tc, 'Facture.etat' => 1), array('Facture.id' => $id_fac));
                ...
                if (!empty($this->request->data['Lignereglementimpaye'])) {
                    foreach ($this->request->data['Lignereglementimpaye']as $j => $l) {
                        if ($mntt > 0 && array_key_exists('piecereglementclient_id', $l) && $l['Montant']!='') {
                          $li = array();
                           $li['reglement_id'] = $reg_id;
                           $li['piecereglement_id'] = $l['piecereglementclient_id'];
                          // $li['tauxchange'] = @$l['tauxchange']; 
                           $id_piece = $l['piecereglementclient_id'];
                           $li['Montant'] = $l['Montant'];
                            $montantaffecter = $montantaffecter + $l['Montant'];
                            $this->Piecereglement->updateAll(array('Piecereglement.reglement ' => $reg_id, 'Piecereglement.mantantregler' => 'Piecereglement.mantantregler+' . $l['Montant']), array('Piecereglement.id' => $id_piece));
                            $this->Lignereglement->create();
                            $this->Lignereglement->save($li);
                        }
                    }
                ...
                $this->Reglement->updateAll(array('Reglement.montantaffecte' => $montantaffecter), array('Reglement.id' => $reg_id));
...
//                            }
        ...
        if (!empty($fournisseur_id)) {
            ...
            $carnetcheques = $this->Carnetcheque->find('list', array('fields' => array('Carnetcheque.numero'),'conditions' => array('Carnetcheque.fermer' =>0)));
            ...
            $situat = "Impayé";
            $impayes = $this->Piecereglement->find('all', array(
                'conditions' => array(
                    'Reglement.fournisseur_id'=> $fournisseur_id,
                    'Piecereglement.situation =' . "'" . $situat . "'", 'Piecereglement.montant>Piecereglement.mantantregler')
                , 'recursive' => 0));
            ...
            //debug($facture);die;
            if ($devisefournisseur == 1) {
                $paiements = $this->Paiement->find('list', array('conditions' => array('Paiement.id in (1,2,3,4,5,8)')));
            } else {
...
//                }  
        ...
        $pointdeventes = $this->Pointdevente->find('list', array('conditions' => array(@$cond_liste_pv)));
        $this->set(compact('impayes','depots', 'pointdeventes', 'p', 'regles', 'namefournisseur', 'etatpiecereglements', 'mm', 'importations', 'devisefournisseur', 'fournisseurs', 'fournisseur_id', 'facture', 'paiements', 'carnetcheques', 'valeurs', 'comptes'));
    }
...
//            debug($this->request->data);die;
            ...
            if ($this->Reglement->save($this->request->data)) {
                ...
                if (!empty($lignesreg)) {
                    foreach ($lignesreg as $k => $ligne) {
                        if (!empty($ligne['Lignereglement']['facture_id'])) {
                        ...
                        if (!empty($ligne['Lignereglement']['piecereglement_id'])) {
                            $this->Piecereglement->updateAll(array('Piecereglement.reglement' =>0, 'Piecereglement.mantantregler' => 'Piecereglement.mantantregler-' . $ligne['Lignereglement']['Montant'],
                                    ), array('Piecereglement.id' => $ligne['Lignereglement']['piecereglement_id']));
                        }
                    }
                ...
                if (!empty($this->request->data['Lignereglementimpaye'])) {
                    foreach ($this->request->data['Lignereglementimpaye']as $j => $l) {
                        if ($mntt > 0 && array_key_exists('piecereglement_id', $l) && $l['Montant']!='') {
                          $li = array();
                           $li['reglement_id'] = $reg_id;
                           $li['piecereglement_id'] = $l['piecereglement_id'];
                          // $li['tauxchange'] = @$l['tauxchange']; 
                           $id_piece = $l['piecereglement_id'];
                           $li['Montant'] = $l['Montant'];
                            $montantaffecter = $montantaffecter + $l['Montant'];
                            $this->Piecereglement->updateAll(array('Piecereglement.reglement ' => $reg_id, 'Piecereglement.mantantregler' => 'Piecereglement.mantantregler+' . $l['Montant']), array('Piecereglement.id' => $id_piece));
                            $this->Lignereglement->create();
        ...
        if ($fournisseur_id) {
            $t = '0';
            $p= '0';
            //debug($this->request->data['Lignereglement']);
            foreach ($this->request->data['Lignereglement']as $j => $l) {
                if (!empty($l['facture_id'])) $t = $t . ',' . $l['facture_id'];
                else  $p = $p . ',' . $l['piecereglement_id'];
            }
            $t= $t . ',0';
            $p= $p . ',0';
            //debug($t);
            ...
            $impayes = $this->Piecereglement->find('all', array(
                'conditions' => array(
                    'Reglement.fournisseur_id'=> $fournisseur_id,
                    'Piecereglement.situation =' . "'" . $situat . "'", '(Piecereglement.montant>Piecereglement.mantantregler or Piecereglement.id in(' . $p . ') )')
                , 'recursive' => 0));
        ...
        $regles[1] = "Non Reglé";
        $regles[2] = "Reglé";
        $this->set(compact('regles', 'id', 'impayes', 'namefournisseur', 'etatpiecereglements', 'importations', 'devisefournisseur', 'credit', 'fournisseurs', 'fournisseur_id', 'fournisseur', 'facture', 'paiements', 'carnetcheques', 'valeurs', 'date', 'montant', 'facreg', 'totalfacture', 'piecesreg', 'lignesreg', 'retenue', 'comptes'));
    }
    ...
    public function delete($id = null) {
        ...
        $pvansar = $abcd['Reglement']['pointdevente_id'];
        if ($this->Reglement->delete()) {
            $this->misejour("Reglement", $numansar, $id,$pvansar);
...
//        }
        ...
        if (!empty($this->request->query['exercice_id']) && $cond1=='' && $cond='') {
            $exerciceid = $this->request->query['exercice_id'];
            $conde = 'Reglementclient.exercice_id =' . $exercices[$exerciceid];
            $cond ='';
            $cond1='';
        }
    ...
    public function imputereglements($val = null, $id = null) {
        $this->layout = null;
        $this->Reglement->updateAll(array(
        'Reglement.impute' =>$val), array('Reglement.id' => $id));
        die;
    }
