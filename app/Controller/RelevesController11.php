...
class RelevesController extends AppController {
	...
	public function index() {
        ...
        if ($this->request->data['Recherche']['personnel_id']) {
            ...
            @$condbr1,@$condbr2,@$condr3,@$condr4,@$cond5,$condbb1),'recursive'=>0));
            $soldereg=$this->Reglementclient->find('first', array(
            'fields'=>array('sum((Reglementclient.Montant)) as solde'),    
            'conditions' => array(@$condbbs,$condr3),'recursive'=>0 ));
            if(!empty($soldereg)){
                $solde=$solde-$soldereg[0]['solde'];
            }
            $piecereglements = $this->Piecereglementclient->find('all', array('conditions' => array(
            'Piecereglementclient.paiement_id in (2,3)','Piecereglementclient.reglement=0','Piecereglementclient.situation="Impayé"',@$condr1,@$condr2,@$condr3,@$condr4,@$cond5,$condbb1),'recursive'=>0));   
             $soldepiece=$this->Piecereglementclient->find('first', array(
            'fields'=>array('sum(Piecereglementclient.montant) as solde'),    
            'conditions' => array(@$condbbs,$condr3,'Piecereglementclient.paiement_id in (2,3)','Piecereglementclient.reglement=0','Piecereglementclient.situation="Impayé"'),'recursive'=>0 ));
            if(!empty($soldepiece)){
