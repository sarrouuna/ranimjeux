...
class CarnetchequesController extends AppController {
    ...
    public function add() {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Carnetcheque->save($this->request->data)) {
                ...
                $this->misejour("Carnetcheque", "add", $id);
    ...
    public function edit($id = null) {
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if ($this->Carnetcheque->save($this->request->data)) {
                $this->misejour("Carnetcheque", "edit", $id);
//debug($this->request->data);
                $this->Cheque->deleteAll(array('Cheque.carnetcheque_id' => $id), false);
                for ($i = $this->request->data['Carnetcheque']['debut']; $i <= $this->request->data['Carnetcheque']['debut']+$this->request->data['Carnetcheque']['nombre']; $i++) {
...
//die;
    ...
    public function delete($id = null) {
        ...
        $this->Cheque->deleteAll(array('Cheque.carnetcheque_id' => $id), false);
$abcd = $this->Carnetcheque->find('first', array('conditions' => array('Carnetcheque.id' => $id), 'recursive' => -1));
        $numansar=$abcd['Carnetcheque']['numero'];
        if ($this->Carnetcheque->delete()) {
            $this->misejour("Carnetcheque", $numansar, $id);
    ...
    public function  fermercarnetcheque($val=null,$id=null){
    $this->layout = null;
    $this->Carnetcheque->updateAll(array(
    'Carnetcheque.fermer' =>$val), 
    array('Carnetcheque.id' => $id));       
    } 
