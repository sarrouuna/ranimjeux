...
class InventairesController extends AppController {
    ...
    public function imprimer($id = null) {
        ...
        $articles = $this->Article->find('list');
        $ligneinvents = $this->Ligneinventaire->find('all', array(
            'conditions' => array('Ligneinventaire.inventaire_id' => $id), 'order' => array('Ligneinventaire.id' => 'ASC')
        ));
        $this->set(compact('ligneinvents', 'articles'));
    }
                    ...
                    foreach ($this->request->data['Ligneinventaire'] as $ligneinventaire) {
                        if ($ligneinventaire['article_id'] != "") {
                            if ($ligneinventaire['sup'] != 1) {
                                ...
                                $prixstocks = $this->Lignecopiestock->find('all', array('conditions' => array('Copiestockdepot.inventaire_id' => $id,
                                        'Lignecopiestock.article_id' => $ligneinventaire['article_id'])));
                                $ligneinventaire['coutderevien'] = 0;
                                if (!empty($prixstocks)) {
                                    foreach ($prixstocks as $prixst) {
                                        if (!empty($prixstocks['Lignecopiestock']['prix'])) {
                                            $ligneinventaire['prix'] = $prixstocks['Lignecopiestock']['prix'];
                                            $ligneinventaire['coutderevien'] = $prixstocks['Lignecopiestock']['prix'];
                                        }
                                    }
                                }


//                            $lignecopiestock = $this->Lignecopiestock->find('first', array('conditions' => array('Lignecopiestock.article_id'=>$ligneinventaire['article_id'])));
//                            //debug($lignecopiestock);die;
//                            if(!empty($lignecopiestock)){
//                            $ligneinventaire['prix'] = $lignecopiestock['Lignecopiestock']['prix'];
//                            $ligneinventaire['coutderevien'] = $lignecopiestock['Lignecopiestock']['prix'];
//                            }else{
//                            $ligneinventaire['prix'] = 0;
//                            $ligneinventaire['coutderevien'] = 0;    
//                            }
                                ...
                                $ligneinventaire['id'] = '';
                                //if($valide==1){
                                //$ligneinventairetest = $this->Ligneinventaire->find('first', array('conditions' => array('Ligneinventaire.article_id'=>$ligneinventaire['article_id'],'Ligneinventaire.inventaire_id'=>$id)));
                                ...
                                //$this->Stockdepot->updateAll(array('Stockdepot.quantite' => 'Stockdepot.quantite+'.$ligneinventaire['quantite']), array('Stockdepot.id' =>$stckdepot[0]['Stockdepot']['id']));
                                //$this->Stockdepot->updateAll(array('Stockdepot.quantite' => 'Stockdepot.quantite+'.$ligneinventaire['quantite']), array('Stockdepot.id' =>$stckdepot[0]['Stockdepot']['id']));
                                //}
                            }
                            //else {
                    ...
                    if (!empty($copiestock)) {
                        $restelignecopiestock = $this->Lignecopiestock->find('all', array('conditions' => array('Lignecopiestock.copiestockdepot_id' => $copiestock['Copiestockdepot']['id'], 'Lignecopiestock.depot_id' => $depotid, 'Lignecopiestock.existe' => 0)));
...
//                    debug($ligneinventaire);die;
                    if ($ligneinventaire['sup'] = '') {
                        ...
                        //debug($ligneinventaire['quantite']);die;
...
//                            $this->Stockdepot->save($newstock);
                        ...
                        //$this->Ligneinventaire->updateAll(array('Ligneinventaire.quantite' => 'Ligneinventaire.quantite+'.$ligneinventaire['quantite']), array('Ligneinventaire.id' =>$ligneinventairetest['Ligneinventaire']['id']));
                        //$this->Stockdepot->updateAll(array('Stockdepot.quantite' => 'Stockdepot.quantite+'.$ligneinventaire['quantite']), array('Stockdepot.id' =>$stckdepot[0]['Stockdepot']['id']));
                        //$stckdepots = $this->Stockdepot->find('first', array('conditions' => array('Stockdepot.depot_id' =>$ligneinventaire['depot_id'],'Stockdepot.article_id'=>$this->request->data['Inventaire']['article_id']),'recursive'=>-1));
                        //$this->Stockdepot->updateAll(array('Stockdepot.quantite' => 'Stockdepot.quantite+'.$ligneinventaire['quantite']), array('Stockdepot.id' =>$stckdepots['Stockdepot']['id']));
                        //}
                    }
                }
                //$copiestock = $this->Copiestockdepot->find('first', array('conditions' => array('Copiestockdepot.inventaire_id' => $id)));
            ...
            if ($this->Inventaire->save($this->request->data)) {
                ...
                if (isset($this->request->data['Ligneinventaire'])) {
                    ...
                    if ($valide == 1) {
                        ...
                        //$teststock = $this->Stockdepot->find('first', array('conditions' => array('Stockdepot.depot_id' => $depotid)));
                        //debug($teststock);
                    }
                    foreach ($this->request->data['Ligneinventaire'] as $ligneinventaire) {
                        if($ligneinventaire['article_id']!=""){
                        if ($ligneinventaire['sup'] != 1) {
                            $ligneinventaire['inventaire_id'] = $id;
                            $ligneinventaire['depot_id'] = $depotid;
                            $prixstocks = $this->Lignecopiestock->find('all', array('conditions' => array('Copiestockdepot.inventaire_id' => $id, 'Lignecopiestock.article_id' => $ligneinventaire['article_id'])));
                            $ligneinventaire['prix'] = 0;
                            $ligneinventaire['coutderevien'] = 0;
                            if (!empty($prixstocks)) {
                                foreach ($prixstocks as $prixst) {
...
//                            $lignecopiestock = $this->Lignecopiestock->find('first', array('conditions' => array('Copiestockdepot.inventaire_id' => $id,'Lignecopiestock.article_id'=>$ligneinventaire['article_id'])));
//                            //debug($lignecopiestock);die;
//                            if(!empty($lignecopiestock)){
//                            $ligneinventaire['prix'] = $lignecopiestock['Lignecopiestock']['prix'];
//                            $ligneinventaire['coutderevien'] = $lignecopiestock['Lignecopiestock']['prix'];
...
//                            }
                            ...
                            if ($valide == 1) {
                                ...
                                //debug($ligneinventairetest);die;
                                if (empty($ligneinventairetest)) {
                                    //debug($ligneinventaire);   
                                    $this->Stockdepot->create();
                                    $this->Stockdepot->save($ligneinventaire);
                    ...
                    $restelignecopiestock = $this->Lignecopiestock->find('all', array('conditions' => array('Lignecopiestock.depot_id' => $depotid, 'Lignecopiestock.existe' => 0), 'order' => array('Copiestockdepot.id' => 'desc')));
                    //debug($restelignecopiestock) ;die; 
                    if (!empty($restelignecopiestock)) {
                        foreach ($restelignecopiestock as $rest) {
                            $testlignebonecart = $this->Bonecart->find('first', array('conditions' => array('Bonecart.inventaire_id' => $id, 'Bonecart.depot_id' => $depotid, 'Bonecart.article_id' => $rest['Lignecopiestock']['article_id'])));
                            if (!empty($testlignebonecart)) {
                                $bonecart['id'] = $testlignebonecart['Bonecart']['id'];
                            } else {
                                $bonecart['id'] = "";
        ...
        $this->set(compact('valide', 'depots', 'articles', 'ligneinvents', 'day'));
    }
