   ...
   // production ajouter des packer de livraison 
    ...
    bootbox.confirm("voulez vous liver un packer a l'article  ?", function(result) {
     ...
     $('.produit').each(function(){
        ...
        if(ind==index){
            //alert(ind);
            valeur=$('#qte_nnliv'+ind).val();//alert(valeur);
            anc=$('#qte_liv'+ind).val();
...
$('.clientreglement,.marquereglement').on('change',function(){
val=$('.clientreglement').val()||0; 
val2=$('.marquereglement').val()||0; 

//alert(val);
if(val!=0 && val2!=0)
$(location).attr('href',wr+"Reglements/add/"+val+'/'+val2);
