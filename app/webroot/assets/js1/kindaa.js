...
$('.sup').on('click',function(){
    ...
    $('.sup').each(function(){
        ...
        if(ind==index){
           ...
           $('#sup'+index).val('1'); 
           $('#total'+index).html('0');  
           sommetotal(); // exclusive aux pages entreemp parametre: class total et somme
        }
    })
        }
...
// remove les labels de table matiere premiere
$('.nolabel').find('label').each(function(){
                $(this).remove(); 
       })
...
$("#EntreempFournisseurId").on('change',function(){
//       --- bon commande client adresse -----
//alert();
       //$('.adrcli').html("");
       $(".ajouterligne").show();
       ...
       $.ajax({
            type: "POST",
            url: wr+"Entreemps/select/"+fourid+"/"+indexmax,
            dataType : "html"
        }).done(function(data){
            console.log(data);
          $("#divart0").html('');   
        ...
        uniform_select('EntreempFournisseurId');
        $('.cc').remove();
   }) 
 
   $(".cal_prix").on('keyup',function(){ 
 lindex=$(this).attr('index');
   // alert(lindex); 
   prix=$("#ttc"+lindex).val();
   qte=$("#qte"+lindex).val();
   total=qte*prix;
//alert(total.toFixed(3));
   $("#total"+lindex).html(total.toFixed(3));
sommetotal();
   }) 
...
function cal_prix(fourid,lindex){
    artid=$('#matierepremiere_id'+lindex).val();
    //alert(fourid+" "+lindex+" "+artid); 
     $('#ttc'+lindex).html("");
     $('#total'+lindex).html(""); 
       $.ajax({
            type: "POST",
            url: wr+"Entreemps/select2/"+fourid+"/"+lindex+"/"+artid,
            dataType : "html"
        }).done(function(data){
          $('#ttc'+lindex).val(data); 
         // uniform_select('BoncommandeLigneclientId');
