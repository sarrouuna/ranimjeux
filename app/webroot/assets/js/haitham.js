...
//             alert("d5al");
            ...
            if(data.tvaclient!=null){
             ...
             $('#tva1' + index).val(data.tvaclient);
            }else{
             $('#tva' + index).val(data.tva);
             $('#tva1' + index).val(data.tva);
            }
...
function articleidblcodebare(index) {
    //alert("helmi"+index);
    // fournisseur_id== $('#fc').val();
    amine = $('#amine').val() || 0;
    //index=$(this).attr('index');
...
//           alert(trans_remise);
    $.ajax({
        type: "POST",
        url: wr + "Bonlivraisons/articlecode/" + code + "/" + depot_id + "/" + client_id + "/" + vente + "/" + fournisseur_id,
        dataType: "json",
        async: false,
        global: false //}l'envoie'
    }).done(function (data) {
...
//        alert($('#code' + index).val());
        //$('#code' + index).val("");
		            $('#article_id' + index).val(data.articleid);
					next=Number(index)+1;

        $('#quantite' + index).val(1);
        $('#pmp' + index).val("");
        $('#tva' + index).val("");
        $('#puttc' + index).val("");
        $('#prixhtva' + index).val("");
