...
alert();
    ...
    $('.recalculer_par_typeclient').on('change', function () {
	 ...
	 //************************************************************************
    $('.recalculer_par_typetimbre').on('change', function () {
       
       timbre= $(this).val();
        $('#typetimbre').val(timbre);
      
		
        calculefacture();
    });
	
	 $('.recalculer_par_typefodec').on('change', function () {
...
//******************hafedh*****************/
		maxbonliv = $('#maxbonliv').val();
       // alert(Number(maxbonliv));
        for (i = 0; i <= Number(maxbonliv); i++) {
			if ($('#bonliv_id' + i).is(':checked')) {//alert();
				testt = true;
				ttbl = Number($('#bonliv_id' + i).attr('mnt')) + Number(ttbl);
			}
		}
...
//                    } else {
	...
	//desactivé par sirine code barre
	 if ($(this).attr('champ') == 'designation') {
                    $(this).attr('onkeypress', 'fuckfocus("input","' + ind + '","' + lename + '")');
                    $(this).attr('onFocus', 'select();');
	 }
				  if ($(this).attr('champ') == 'code') {
                    $(this).attr('onkeyup', ' articleidblcodebare("' + ind + '");');
                  }
...
function autocomplete_name_clients(inp_name,inp_id) {
  /*the autocomplete function takes two arguments,
  the text field element and an array of possible autocompleted values:*/
    inp=document.getElementById(inp_name);
    id=document.getElementById(inp_id);
