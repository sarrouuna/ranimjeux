...
<!--                    <li><a class="refresh" href="javascript:void(0)"><i class="fa fa-refresh"></i></a></li>
        ...
        <?php echo $this->Form->create('Relevefournisseur',array('autocomplete' => 'off','class'=>'form-horizontal ls_form','id'=>'defaultForm','data-bv-message'=>'This value is not valid','data-bv-feedbackicons-valid'=>'fa fa-check','data-bv-feedbackicons-invalid'=>'fa fa-bug','data-bv-feedbackicons-validating'=>'fa fa-refresh')); ?>
		...
		echo $this->Form->input('Date_debut',array('label'=>'Date début','div'=>'form-group','between'=>'<div class="col-sm-10">','after'=>'</div>','class'=>'form-control datePickerOnly','required data-bv-notempty-message'=>'Champ Obligatoire','type'=>'text') );
                echo $this->Form->input('fournisseur_id',array('div'=>'form-group','between'=>'<div class="col-sm-10">','after'=>'</div>','class'=>'form-control','required data-bv-notempty-message'=>'Champ Obligatoire','empty'=>'Veuillez choisir..','id'=>'FournisseurReleve') );			
  ...
  <div class="col-md-6"> 
	...
	echo $this->Form->input('reg',array('label'=>'Reglé/Non reglé','div'=>'form-group','between'=>'<div class="col-sm-10">','after'=>'</div>','class'=>'form-control','required data-bv-notempty-message'=>'Champ Obligatoire','empty'=>'Veuillez choisir..') );
       ...
       <a  onClick="flvFPW1(wr+'Relevefournisseurs/imprimerrecherche?Date_debut=<?php echo @$Date_debut;?>&Date_fin=<?php echo @$Date_fin;?>&reg=<?php echo @$reg;?>&fournisseur_id=<?php echo @$fournisseur_id;?>&netapayer='+document.getElementById('netapayer').value,'UPLOAD','width=800,height=1150,scrollbars=yes',0,2,2);return document.MM_returnValue" href="javascript:;" ><button class="btn btn-primary">Imprimer</button> </a>
                        ...
                        <div class="col-md-12">
                            <div class="panel panel-default">
                                ...
                                <div class="panel-body">
                                    ...
                                    <?php    if(!empty($relevefournisseurs)){ ?>
                                        <table class="table">
                                            <tr>
                                                <td align="right" width="50%"><b>
                                                  <?php if ($Date_debut == '') echo "Solde initial"; else echo "Solde au ".date("d/m/Y",strtotime(str_replace('-','/',$Date_debut))); ?>  
                                                </b></td>
                                                <td>
                                                   <?php echo sprintf('%.3f',$solde_initial); ?> 
                                                </td>
                                            </tr>
                                        </table>
                                        <?php } ?>
...
<!--		<td width="16%" align="center"><?php echo ('Remise'); ?></td>-->
	...
	<?php $solde = @$solde_initial; $totremise=0;$totBL=0; $totREG=0; $remisereg=0;$cadreB='';
        //debug($relevefournisseurs);die;
        if(!empty($relevefournisseurs)){
        foreach (@$relevefournisseurs as $relefe):
            $montant="";$acompte="";
            if(@$relefe['Relevefournisseur']['Type']=='BL' || @$relefe['Relevefournisseur']['Type']=='BLnoir' || @$relefe['Relevefournisseur']['Type']=='FAC'){
                $solde=$solde+$relefe[0]['Montant'];
                $totBL=$totBL+$relefe[0]['Montant'];
                $cadreB='';
                $montant=$relefe[0]['Montant'];
            }
            if(@$relefe['Relevefournisseur']['Type']=='REG' || @$relefe['Relevefournisseur']['Type']=='REGnoir' || @$relefe['Relevefournisseur']['Type']=='AV-FAC'){
                $solde=$solde-$relefe[0]['Montant'];
                $totREG=$totREG+$relefe[0]['Montant'];
                $cadreB='style="background-color:#B9CDCA;"';
                $acompte=$relefe[0]['Montant'];
            }
            ...
            ?>
	<tr>
                <td width="12%" <?php echo @$cadreB ?>align="left"><?php echo h($relefe['Relevefournisseur']['Type']) ;echo h($relefe['Relevefournisseur']['Numero']); ?></td>
		<td width="12%" <?php echo @$cadreB ?> align="center"><?php 
                $Date=$relefe['Relevefournisseur']['Date'];
                $Date=date('d/m/Y', strtotime(str_replace('-', '/',$Date)));
                echo $Date;
                ?></td>
<!--		<td><?php echo h($relefe['Relevefournisseur']['Client']); ?></td>-->
		<!--<td width="12%" <?php echo $cadreB ?> align="center"><?php echo h($relefe['Relevefournisseur']['Contact']); ?></td>-->
                <td width="12%" <?php echo $cadreB ?> align="center"><?php echo h($relefe['Relevefournisseur']['mode']); ?></td>
...
<!--		<td width="16%" <?php echo $cadreB ?> align="right"><?php echo h($relefe['Relevefournisseur']['Remise']); ?></td>-->
