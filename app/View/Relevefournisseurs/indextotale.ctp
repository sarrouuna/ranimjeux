...
<!--                    <li><a class="refresh" href="javascript:void(0)"><i class="fa fa-refresh"></i></a></li>
		...
		echo $this->Form->input('date1',array('label'=>'Date début','div'=>'form-group','between'=>'<div class="col-sm-10">','after'=>'</div>','class'=>'form-control datePickerOnly','required data-bv-notempty-message'=>'Champ Obligatoire','type'=>'text') );
                echo $this->Form->input('fournisseur_id',array('label'=>'client début','id'=>'client_id','div'=>'form-group','between'=>'<div class="col-sm-10">','after'=>'</div>','class'=>'form-control','required data-bv-notempty-message'=>'Champ Obligatoire','empty'=>'Veuillez choisir..') );			
  ...
  <div class="col-md-6"> 
        ...
        echo $this->Form->input('fournisseur2_id',array('label'=>'client fin','id'=>'client2_id','div'=>'form-group','between'=>'<div class="col-sm-10">','after'=>'</div>','class'=>'form-control','required data-bv-notempty-message'=>'Champ Obligatoire','empty'=>'Veuillez choisir..') );			
...
?>
                ...
                <th style="border:1px solid black;" bgcolor="#F2D7D5" ><strong><center>Fournisseur</center></strong></th>
	...
	<?php //debug($lignecommandes);
        ...
        $soldetotr=0;
        $arraydate=array();
        foreach ($all_frss as $i=>$all_frs){
        $test="";   
        $debit=0;
        $credit=0;
        $solde=0; 
        $fournisseurid= $all_frs['Fournisseur']['id'];
        $condb3 = 'Bonreception.fournisseur_id ='.$fournisseurid;
        $condf3 = 'Facture.fournisseur_id ='.$fournisseurid;
        $condfa3='Factureavoirfr.fournisseur_id ='.$fournisseurid;
        $condr3 = 'Reglement.fournisseur_id ='.$fournisseurid;    
        $solde=$all_frs['Fournisseur']['solde'];
        //$solder=$all_client['Client']['solderecouvrement'];
        
        //debug($solder);
        
            $soldeavoir=ClassRegistry::init('Factureavoirfr')->find('first', array(
            'fields'=>array('sum(Factureavoirfr.totalttc) as solde','max(Factureavoirfr.date) as date'),    
            ...
            $soldebl=ClassRegistry::init('Bonreception')->find('first', array(
            'fields'=>array('sum((Bonreception.totalttc)) as solde','max(Bonreception.date) as date'),    
            'conditions' => array(@$condb1,@$condb2,$condb3,'Bonreception.facture_id'=>0),'recursive'=>0 ));
            if(!empty($soldebl)){
                $solde=$solde+$soldebl[0]['solde'];
                $debit=$debit+$soldebl[0]['solde'];
                $arraydate[2]=$soldebl[0]['date'];
            }
            $soldefac=ClassRegistry::init('Facture')->find('first', array(
            'fields'=>array('sum((Facture.totalttc)) as solde','max(Facture.date) as date'),    
            ...
            $soldereg=ClassRegistry::init('Reglement')->find('first', array(
            'fields'=>array('sum((Reglement.Montant)) as solde','max(Reglement.Date) as date'),    
            'conditions' => array(@$condbb1,@$condbb2,$condr3),'recursive'=>0 ));
            if(!empty($soldereg)){
                $solde=$solde-$soldereg[0]['solde'];
                $credit=$credit+$soldereg[0]['solde'];
                $arraydate[4]=$soldereg[0]['date'];
            }
            $soldepiece=ClassRegistry::init('Piecereglement')->find('first', array(
            'fields'=>array('sum(Piecereglement.montant) as solde','max(Reglement.Date) as date'),    
            'conditions' => array(@$condss1,@$condss2,$condr3,'Piecereglement.etatpiecereglement_id=4'),'recursive'=>0 ));
            if(!empty($soldepiece)){
...
//         }
	...
	<tr><td align="left" style="border:1px solid black;">
            <?php 
                $societe = ClassRegistry::init('Societe')->find('first', array('conditions' => array('Societe.composantsoc'=>CakeSession::read('composantsoc'))));
                
                echo $this->Html->link($all_frs['Fournisseur']['code'], array('controller' => 'relevefournisseurs', 'action' => 'index', $all_frs['Fournisseur']['id'],$societe['Societe']['id'])); ?>
		</td>
		<td align="left" style="border:1px solid black;"><?php echo $all_frs['Fournisseur']['name']  ;?></td>
