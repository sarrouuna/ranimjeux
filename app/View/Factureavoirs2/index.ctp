...
foreach($lien as $k=>$liens){
	if(@$liens['lien']=='factureavoirs'){
		...
		$imprimer=$liens['imprimer'];
                            ...
                            $lienn=  CakeSession::read('lien_achat');
                            foreach($lienn as $k=>$lienns){
                                    if(@$lienns['lien']=='bonentres'){
                                                   $addbonentre=$lienns['add'];
                                           }
                            } 
} 
?>
...
<div class="row">
    <div class="col-md-12">
        <a class="btn btn btn-danger" href="<?php echo $this->webroot;?>Factureavoirs/add"/> <i class="fa fa-plus-circle"></i> Ajouter facture à voir financier </a>
        <a class="btn btn btn-danger" href="<?php echo $this->webroot;?>Factureavoirs/addfactureavoir"/> <i class="fa fa-plus-circle"></i> Ajouter facture à voir </a>
    </div>
</div>
...
<div class="row" >
    <div class="col-md-12" >
        ...
        <?php echo $this->Form->create('Factureavoir',array('autocomplete' => 'off','class'=>'form-horizontal ls_form','id'=>'defaultForm')); ?>
...
<div class="row">
	...
	<tr>
		...
		<th><?php echo $this->Paginator->sort('Facture client'); ?></th>
	         	         
		<th><?php echo $this->Paginator->sort('Totalttc'); ?></th>
	         
		<th><?php echo $this->Paginator->sort('Numero'); ?></th>
	         
		<th><?php echo $this->Paginator->sort('Typefacture_id'); ?></th>
	...
	<tr>
		...
		<td align="center">
			...
			<?php if(($edit==1)&($factureavoir['Factureavoir']['etat']==0)){ echo $this->Html->link("<button class='btn btn-xs btn-warning'><i class='fa fa-edit'></i></button>", array('action' => 'edit', $factureavoir['Factureavoir']['id']),array('escape' => false)); }?>
			<?php if(($delete==1)&($factureavoir['Factureavoir']['etat']==0)){ echo $this->Form->postLink("<button class='btn btn-xs btn-danger'><i class='fa fa-trash-o'></i></button>", array('action' => 'delete', $factureavoir['Factureavoir']['id']),array('escape' => false,null), __('Veuillez vraiment supprimer cette enregistrement # %s?', $factureavoir['Factureavoir']['id'])); }?>
		<?php if($imprimer==1) {
