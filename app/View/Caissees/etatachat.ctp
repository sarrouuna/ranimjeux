		   ...
		   <?php echo $this->Form->input('moi_id',array('id'=>'moi_id','multiple'=>'multiple','empty'=>'veuillez choisir','div'=>'form-group','label'=>'Mois','between'=>'<div class="col-sm-10">','after'=>'</div>','class'=>'form-control'));?>
               ...
               <div class="col-md-6">
                ...
                <div class="form-group">
                    ...
                    <?php // if($imprimer==1){ ?>
      <a  onClick="flvFPW1(wr+'Caissees/imprimeretatachat?annee=<?php echo @$annee;?>&moiid=<?php echo @$moiid;?>','UPLOAD','width=800,height=1150,scrollbars=yes',0,2,2);return document.MM_returnValue" href="javascript:;" ><button class="btn btn-primary">Imprimer</button> </a>
                    <?php //}?>
...
<div class="row">
                        <div class="col-md-12">
                            <div class="panel panel-default">
                                <div class="panel-heading">
                                    <h3 class="panel-title"><?php echo __('Etat des achats'); ?></h3>
                                </div>
                                <div class="panel-body">
	...
	<tr>
		<td ><?php echo h(date("d/m/Y",strtotime(str_replace('-','/',$facture['Facture']['date'])))); ?>&nbsp;</td>
                ...
                <td ><?php $year=explode( '-',$facture['Facture']['date']);
                echo $this->Html->link($facture['Facture']['numero'], array('controller' => 'factures', 'action' => 'view', $facture['Facture']['id'])); echo(' / '.$year[0]) ;?>
                 </td>
		<td ><?php echo h($facture['Facture']['totalht']); ?>&nbsp;</td>
                <td ><?php echo h($facture['Facture']['tva']); ?>&nbsp;</td>
                <td ><?php echo h($facture['Timbre']['timbre']); ?>&nbsp;</td>
		<td ><?php echo h($facture['Facture']['totalttc']); ?>&nbsp;</td>
		<td >
                 <table class="table table-bordered table-striped table-bottomless" id="addtable" style="width:100%" align="center" >
                  <?php if(!empty($lignereglements[$facture['Facture']['id']])){ foreach(@$lignereglements[$facture['Facture']['id']][0]['Reglement']['Piecereglement'] as $piece){ 
                       $banque="  " ; if(!empty($piece['compte_id'])){$banque=$comptes[$piece['compte_id']];}         ?>
                   <tr>
		     <td ><?php echo h($paiements[$piece['paiement_id']]); ?>&nbsp;</td>
		     <td ><?php echo h($piece['num']); ?>&nbsp;</td>
		     <td ><?php echo h($piece['montant']); ?>&nbsp;</td>
		     <td ><?php echo h( $banque); ?>&nbsp;</td>
                   </tr>
                  <?php } }?>
                  </table>
                </td>
		<td ><?php echo h($facture['Facture']['totalttc']-$facture['Facture']['Montant_Regler']); ?>&nbsp;</td>
	</tr>
<?php endforeach; ?>
