...
if ($tablignefactures != array()) {
   ...
   <tr  bgcolor="#CCCCCC" align="center">
        ...
        if ($br['type'] == 'Facture client') {
            ...
            if (floatval($exono) != 0) {
                $factzero = $objfact->find('all', array('fields' => array('SUM(Lignefactureclient.totalht) as mtva', 'Lignefactureclient.tva as taux'), 'conditions' => (array('Lignefactureclient.factureclient_id' => $br['id_piece'])), 'recursive' => -1));
            } else {
                $factzero = $objfact->find('all', array('fields' => array('SUM(Lignefactureclient.totalht) as mtva', 'Lignefactureclient.tva as taux'), 'conditions' => (array('Lignefactureclient.factureclient_id' => $br['id_piece'], 'Lignefactureclient.tva' => 0)), 'recursive' => -1));
            }
            if ($factzero[0][0]['mtva'] != NULL) {
                $f = $factzero[0][0]['mtva'];
            }
            foreach ($tvas as $t) {
...
//debug($fact);
                if ($fact[0][0]['mtva'] != NULL && floatval($exono) == 0) {
...
<th width="5%" align="center" $zz >' . $br['code'] . '</th>
                        ';
        $tbl.='<th width="5%" align="right" $zz >' . $f . '</th>';
        //debug($listva);die;
        foreach ($listva as $v) { //debug($v); 
            $tbl.='<th width="' . $w . '%" align="right" $zz >' . $v['base'] . '</th> 
                                            <th width="' . $w . '%" align="right" $zz >' . $v['mtva'] . '</th>';
        }
...
$tbl .= '
   <tr  bgcolor="#CCCCCC" align="center">    
        ...
        <td width="41%" colspan="4" nobr="nobr" align="right"  $zz>Total</td>
        ';
$tbl .='<th width="5%" align="right" $zz >' . sprintf("%.3f", $totfactzero) . '</th> ';
foreach ($tf as $y) {//debug($y);die;
    $tbl .='<th width="' . $w . '%" align="right" $zz >' . sprintf("%.3f", $y['base']) . '</th> 
                        <th width="' . $w . '%" align="right" $zz >' . sprintf("%.3f", $y['montant']) . '</th>';
}
$tbl .='<th width="5%" align="right" $zz >' . sprintf("%.3f", $tottimbre) . '</th>
        <th width="10%" align="right" $zz >' . sprintf("%.3f", $totnetapayer) . '</th>
        </tr>
</table>';
