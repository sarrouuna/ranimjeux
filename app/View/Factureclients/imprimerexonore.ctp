...
//debug($br);die();
                if ($fact[0][0]['mtva'] != NULL ) {
                	if($tv='18'){
						$tvmnt18 = floatval($fact[0][0]['mtva']) * floatval($tv) / 100;
            ...
            foreach ($tvas as $t) {
				...
				if($tv='18'){
                    ...
                    $listva[$p]['nom'] = 'Base ' . intval(floatval($t['Tva']['name'])) . '%';
                    $listva[$p]['mtva'] = sprintf("%.3f", $tvmnt);
                    $listva[$p]['mtva18'] = sprintf("%.3f", $tvmnt);
                    ...
                    $listvaall[$ccp]['nom'] = 'Base ' . intval(floatval($t['Tva']['name'])) . '%';
                    $listvaall[$ccp]['mtva'] = sprintf("%.3f", $tvmnt);
                    $listvaall[$ccp]['mtva18'] = sprintf("%.3f", $tvmnt);
...
//    die;
	...
	//$totalttccfodec=($br['totalttc']*1%)/100;
...
//      $tottva18 = $tottva18 + $tva18;
    $tbl .=
        ...
        <td width="3%" align="center" heigth="30px" $zz >' . $i . '</td>
                        ...
                        $tbl.='
						<td width="5%" align="right" $zz >' . $br['mtva18'] . '</td>
...
$tbl .= '
   <tr  bgcolor="#CCCCCC" align="center">
        <td width="43%" colspan="6" nobr="nobr" align="right"  $zz>Total</td>
...
foreach ($tf as $y){
 ...
 $tbl .='<td width="' . $w . '%" align="right" $zz >' . sprintf("%.3f", $y['base']) . '</td>
        ...
        $tbl.='<td width="5%" align="right" $zz >' . sprintf("%.3f", $y['montant2']) . '</td>
