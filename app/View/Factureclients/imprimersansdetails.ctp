...
foreach ($tablignefactures as $br) {
    ...
    $totresteg = $totresteg + $br['reste'];
    //debug($codeclt);debug('Code : '.$br['code']);
    if ($br['code'] == $codeclt) {
        $tbl .=
   ...
   <tr  bgcolor="#CCCCCC" align="center">    
        ...
        <td width="90%" colspan="7" nobr="nobr" align="right"  $zz>Total Factures Non réglées </td>
        ';

            $tbl .='<th width="10%" align="center" $zz >' . sprintf("%.3f", $totreste) . '</th>
        </tr>
    ';
            $totreste = 0;
        }
...
$tbl .='<th width="10%" align="right" $zz >' . sprintf("%.3f", $totreste) . '</th>
        </tr>
    ';
$tbl .= '
   <tr  bgcolor="#CCCCCC" align="center">    
       
        <td width="70%" colspan="5" nobr="nobr" align="right"  $zz>Total general </td>
        ';

$tbl .='<th width="10%" align="right" $zz >' . sprintf("%.3f", $totnetapayer) . '</th>
        <th width="10%" align="right" $zz >' . sprintf("%.3f", $totmntregler) . '</th>
            <th width="10%" align="right" $zz >' . sprintf("%.3f", $totresteg) . '</th>
        </tr>
</table>';
...
ob_end_clean();
$pdf->Output('Listefacturesansdetails.pdf', 'I');
