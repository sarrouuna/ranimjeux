 ...
 <table border="1" align="center" cellpadding="2" cellspacing="0"  width="100%" class="table" nobr="true">
   <tr  bgcolor="#CCCCCC" align="center">
                       ...
                       <td width="5%" align="center" $zz ><strong>Code Cli</strong></td>
                        ...
                        <td width="10%" align="center" $zz  ><strong>Nom/Raison</strong></td>';
			foreach ($tvas as $t) { //debug($t);die;
            $tbl.='
            <td width="' . $w . '%" align="center" $zz ><strong>Total HT Brut</strong></td>
            <td width="' . $w . '%" align="center" $zz ><strong>Net HT</strong></td>
            <td width="' . $w . '%" align="center" $zz ><strong>Remise</strong></td>
                        <td width="' . $w . '%" align="center" $zz ><strong>TVA ' . intval(floatval($t['Tva']['name'])) . '%</strong></td>';
        }


        $tbl.='
						<td width="5%" align="center" $zz ><strong>TVA 18%</strong></td>
						<td width="5%" align="center" $zz ><strong>Fodec</strong></td>
						<td width="5%" align="center" $zz ><strong>Timbre</strong></td>
						<td width="10%" align="center" $zz ><strong>Net a payer</strong></td>
                        ';
...
foreach ($tablignefactures as $br) {
    ...
    foreach ($tvas as $t) { //debug($t);die;
		...
		if($tv='18')
		{
			$tvmnt18 = floatval($lignefactavoir[0][0]['mtva']) * floatval($tv) / 100;
		...
		else
		{
			$tvmnt = floatval($lignefactavoir[0][0]['mtva']) * floatval($tv) / 100;
                    ...
                    $listva[$p]['mtva18'] = sprintf("%.3f", $tvmnt18);
                    ...
                    $listvaall[$ccp]['mtva18'] = sprintf("%.3f", $tvmnt18);
...
//    die;
    ...
    $totfodec = $totfodec + $br['fodec'];
    $totremise = $totremise + $br['remise'];
    $c= $v['base']- $br['remise'];
    $totnetht = $totnetht + $c;
...
// 	$tottva18 = $tottva18 + $tva18;
    $tbl .=
        ...
        <td width="10%" align="center" heigth="30px" $zz >' .$br['numero'] . '</td>
                        ...
                        <td width="10%" align="left" $zz  >' . $br['client'] . '</td>
                        ';
                        //debug($listva);die;
                            foreach ($listva as $v) { //debug($v); die;
                                $tbl.='<td width="' . $w . '%" align="right" $zz >' . $v['base'] . '</td>
                                <td width="' . $w . '%" align="right" $zz >' . ($v['base']- $br['remise']) . '</td>
                                <td width="' . $w . '%" align="right" $zz >' . $br['remise'] . '</td>
                                            <td width="' . $w . '%" align="right" $zz >' . $v['mtva'] . '</td>';
                            }


    $tbl.='              <td width="5%" align="right" $zz >' . $v['mtva18']. '</td>
          			  	  <td width="5%" align="right" $zz >' . $br['fodec'] . '</td>
...
foreach ($listvaall as $l){
    ...
    $tf[$l['tva']]['montant2']=$tf[$l['tva']]['montant2']+$l['mtva18'];
...
$tbl .= '
   <tr  bgcolor="#CCCCCC" align="center">
        ...
        <td width="40%" colspan="5" nobr="nobr" align="right"  $zz>Total</td>
        ';
foreach ($tf as $y){//debug($y);die;
                        $tbl .='<td width="' . $w . '%" align="right" $zz >' . sprintf("%.3f", $y['base']) . '</td>
                        <td width="' . $w . '%" align="right" $zz >' . sprintf("%.3f", $totnetht) . '</td>
                        <td width="' . $w . '%" align="right" $zz >' . sprintf("%.3f", $totremise) . '</td>
                        <td width="' . $w . '%" align="right" $zz >' . sprintf("%.3f", $y['montant']) . '</td>';
}
    $tbl .='
		<td width="5%" align="right" $zz >' . sprintf("%.3f", $y['montant2']) . '</td>
		<td width="5%" align="right" $zz >' . sprintf("%.3f", $totfodec) . '</td>
		<td width="5%" align="right" $zz >' . sprintf("%.3f", $tottimbre) . '</td>
        <td width="10%" align="right" $zz >' . sprintf("%.3f", $totnetapayer) . '</td>
