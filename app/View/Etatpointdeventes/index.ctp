...
<?php foreach ($tab as $h=>$action){ ?> 
    pieData[<?php echo $h;?>] = {
        label: "<?= $action['PVname'].' CA : '.$action['tot']; ?>",  
        data: <?= $action['por'] ?>
...
<!--    <div class="col-md-12" >
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3 class="panel-title"><?php echo __('CA DETAILLE'); ?></h3>
            </div>
            <div class="panel-body">
<table class="table">       
                                                   <thead>
                                                   ...
                                                   <tbody>
                                              <?php  $totreg=0; $totres=0; ?>
                                                
	                                           <?php foreach ($tab as $k=>$action): 
                                                $mtreste=$action['totttc']-$action['mtregler'];
                                                $totreg=$totreg+$action['mtregler'];
                                                $totres=$totres+$mtreste;
                                                $porreg=($action['mtregler']/$action['totttc'])*100;
                                                $porres=($mtreste/$action['totttc'])*100;
                                                ?>
	<tr>
		<td><strong><?php echo $action['PVname']; ?></strong></td>
		<td align="right"><?php echo number_format($action['totttc'],3, '.', ' '); ?></td>
                <td align="right"><?php echo number_format($action['por'],3, '.', ' '); ?>%</td>
                <td align="right"><?php echo number_format($action['mtregler'],3, '.', ' '); ?></td>
                <td align="right"><?php echo number_format($porreg,3, '.', ' ') ; ?>%</td>
                <td align="right"><?php echo number_format($mtreste,3, '.', ' '); ?></td>
                <td align="right"><?php echo number_format($porres,3, '.', ' '); ?>%</td>
        ...
        <tr>
                    <td><strong> Total</strong></td>
                    <td  align="right"><?php echo number_format($totaleBLFttc,3, '.', ' '); ?></td>
                    <td  align="right">100</td>
                    <td  align="right"><?php echo number_format($totreg,3, '.', ' '); ?></td>
                    <td align="right"></td>
                    <td  align="right"><?php echo number_format($totres,3, '.', ' '); ?></td>
                    <td align="right"></td>
