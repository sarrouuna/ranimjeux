   ...
   foreach($ligne[$per['Famillearticle']['id']] as $tt)
{
  ...
  $totalfamille=$totalfamille+(($insmoi[0]['Article']['Marge']*($tt[0]['sum(`Ticketcaisseligne`.`montant`)']))/100);

				//debug($insmoi); die;	
					
  $tbl .='   <tr >
 <td align="left" width="275px">'.$p.$insmoi[0]['Article']['Designation'].'</td>
 <td>'.sprintf('%.2f',$tt[0]['sum(`Ticketcaisseligne`.`qte`)']).'</td>
 ...
 <td width="75px">'.sprintf('%.3f',($insmoi[0]['Article']['Marge']*($tt[0]['sum(`Ticketcaisseligne`.`montant`)']))/100).'</td>
    

 </tr>
 ';
 $tstf+=$tt[0]['sum(`Ticketcaisseligne`.`montant`)'];
   }
     $tbl .='  <tr >
 ...
 <td width="75px"><font  color="#FF0000">'.sprintf('%.3f',$totalfamille).'</font></td>
  ...
  $tbl .=' <br><br><table width="750px"> <tr >
