...
<div class="row">
	...
	<tr>
		...
		<th><?php echo $this->Paginator->sort('Prix de vente'); ?></th>
	       
			<th class="actions" align="center"></th>
        </tr></thead><tbody>
	<?php foreach ($totarticles as $article): 
	...
	<tr>
		...
		<td align="center">
			<?php echo $this->Html->link("<button class='btn btn-xs btn-success'><i class='fa fa-search'></i></button>", array('action' => 'view', $article['Article']['id']),array('escape' => false)); ?>
                        <?php if($edit==1) {  echo $this->Html->link("<button class='btn btn-xs btn-warning'><i class='fa fa-edit'></i></button>", array('action' => 'edit', $article['Article']['id']),array('escape' => false)); }?>
                        <?php if(($delete==1) && ($nbtotale==0)) { echo $this->Form->postLink("<button class='btn btn-xs btn-danger'><i class='fa fa-trash-o'></i></button>", array('action' => 'delete', $article['Article']['id']),array('escape' => false,null), __('Veuillez vraiment supprimer cette enregistrement # %s?', $article['Article']['id']));} ?>
		</td>
	</tr>
<?php endforeach; ?>
                          </tbody>
