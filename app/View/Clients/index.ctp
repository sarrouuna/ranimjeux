...
<br>
<?php }?>
  <input type="hidden" id="page" value="index_reglementclient">
<div class="row" >
    <div class="col-md-12" >
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                <div class="col-md-6">
                    ...
                    //echo $this->Form->input('client_id', array('label' => 'Client', 'div' => 'form-group', 'between' => '<div class="col-sm-10">', 'id' => 'client_id', 'after' => '</div>', 'class' => 'form-control select ', 'empty' => 'Veuillez Choisir !!'));
                    ...
                    echo $this->Form->input('matriculefiscale', array('label' => 'matricule fiscale', 'div' => 'form-group', 'between' => '<div class="col-sm-10">', 'id' => 'matriculefiscaleclt', 'after' => '</div>', 'class' => 'form-control'));
                    ?>
                </div>
                <div class="col-md-6">
                    <?php
                    echo $this->Form->input('familleclient_id', array('label' => 'Famille', 'div' => 'form-group', 'between' => '<div class="col-sm-10">', 'id' => 'typeetatclient_id', 'after' => '</div>', 'class' => 'form-control select ', 'empty' => 'Veuillez Choisir !!'));
                    ...
                    echo $this->Form->input('zone_id', array('label' => 'Zone', 'div' => 'form-group', 'between' => '<div class="col-sm-10">', 'id' => 'client_id', 'after' => '</div>', 'class' => 'form-control select ', 'empty' => 'Veuillez Choisir !!'));
                    ?>
                </div>      
...
<div class="row">
	...
	<tr>
		...
		<th style="display:none"><?php echo ('Id'); ?></th>
                ...
                <th ><center><?php echo ('societe'); ?></center></th>
		...
		<th><center><?php echo ('Matricule fiscale'); ?></center></th>
	        <th class="actions" align="center">Actif/Non actif</th> 
...
//                 $id_art=$client['Client']['id'];
//                              $nb=$obj1->query("SELECT SUM(nb) AS nb
//FROM (
//   SELECT count(*) nb FROM  `devis`       WHERE  `client_id` =$id_art UNION ALL
//   SELECT count(*) nb FROM  `commandeclients` WHERE  `client_id` =$id_art UNION ALL
//   SELECT count(*) nb FROM  `bonlivraisons` WHERE  `client_id` =$id_art UNION ALL
//   SELECT count(*) nb FROM  `factureclients` WHERE  `client_id` =$id_art UNION ALL
//   SELECT count(*) nb FROM  `factureavoirs` WHERE  `client_id` =$id_art UNION ALL
//   SELECT count(*) nb FROM  `reglementclients` WHERE  `client_id` =$id_art 
//  
//) AS nb");
//                              $nbtotale=$nb['0'][0]['nb'];
	...
	<tr>
		<td style="display:none"><?php echo h($client['Client']['id']); ?></td>
                <td ><?php echo h($client['Client']['societe']); ?></td>
		...
		<td ><?php echo h($client['Client']['matriculefiscale']); ?></td>
                <td align="center">
                <?php if(CakeSession::read('blocageclient')==1){ ?>    
                <input type="checkbox"  <?php if ($client['Client']['etat']==2){ ?>checked <?php }?> index ="<?php echo $client['Client']['id'] ?>"   class="bloquerclient"/>
                <?php } ?>
                </td>
		<td align="center">
