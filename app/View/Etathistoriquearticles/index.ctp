...
<div class="row" >
    <div class="col-md-12" >
        ...
        <?php echo $this->Form->create('Recherche',array('autocomplete' => 'off','class'=>'form-horizontal ls_form')); ?>
                ...
                <div class="col-md-6">                  
              	<?php 
                if(!empty($this->request->data['Recherche']['date1'])){
                $date= $this->request->data['Recherche']['date1'];   
                }else{
                $date=   '01/01/'.date("Y"); 
                }
		echo $this->Form->input('date1',array('value'=>$date,'div'=>'form-group','between'=>'<div class="col-sm-10">','after'=>'</div>','class'=>'form-control datePickerOnly ','type'=>'text','label'=>'Date de') ); 
		echo $this->Form->input('client_id', array('type'=>'hidden','id' => 'client_id', 'div' => 'form-group', 'between' => '<div class="col-sm-12">', 'after' => '</div>', 'class' => 'form-control'));
                ...
                ?>
                <?php 
                echo $this->Form->input('designation', array('yourid'=>'article_id0','div' => 'form-group', 'placeholder' => 'Designation', 'label' => 'Article', 'index' => '0', 'id' => 'designation0', 'champ' => 'designation', 'between' => '<div class="col-sm-10"><div class="autocomplete" style="width:100%;">', 'after' => '</div></div>', 'class' => 'form-control index_designation_manuel_autocomplete', 'type' => 'text'));
                ?>
                </div>
                <div class="col-md-6">
                <?php
		echo $this->Form->input('date2',array('div'=>'form-group','between'=>'<div class="col-sm-10">','after'=>'</div>','class'=>'form-control datePickerOnly','type'=>'text','label'=>"Jusqu'Ã ") ); 
       		echo $this->Form->input('fournisseur_id',array('id'=>'fournisseur_id','empty'=>'veuillez choisir','div'=>'form-group','label'=>'Fournisseur','between'=>'<div class="col-sm-10">','after'=>'</div>','class'=>'form-control'));
                ...
                <label for="RechercheDate2" class="col-md-2 control-label">Tous les exercices </label>
                <div class="col-sm-10">
                    <input class="icheck-purple" type="checkbox" id="checkRed5" value="actif" name="data[Recherche][tousexercices]" <?php if ($checkbox=="checked"){ ?>checked <?php }?>>
                </div></div>
                </div>      
...
<!--                 <a class="btn btn-primary" href="<?php //echo $this->webroot;?>/index"/>Afficher Tout </a>-->
      ...
      <a  onClick="flvFPW1(wr+'Etathistoriquearticles/imprimerrecherche?clientid=<?php echo @$clientid;?>&date1=<?php echo @$date1;?>&date2=<?php echo @$date2;?>&fournisseurid=<?php echo @$fournisseurid;?>&articleid=<?php echo @$articleid;?>&depotidd=<?php echo @$depotidd;?>&exerciceid=<?php echo @$exerciceid;?>','UPLOAD','width=800,height=1150,scrollbars=yes',0,2,2);return document.MM_returnValue" href="javascript:;" ><button class="btn btn-primary">Imprimer</button> </a>
                   
<!--      <a  onClick="'Etathistoriquearticles/corrigestock?depotid='<?php echo @$depotid;?>'&articleid='<?php echo @$articleid;?>" ><button class="btn btn-primary">*</button> </a>-->
...
<td><center>TOT HT </center></td>
<td><center>TVA%</center></td>
...
$qte_final=0;
    ...
    if($historiquearticle['Historiquearticle']['indice']==10){
        $qte_ent=$qte_ent+sprintf("%.3f",$historiquearticle['Historiquearticle']['qte']);
    }
    if($historiquearticle['Historiquearticle']['mode']=="Entreé"){
        $qte_ent=$qte_ent+sprintf("%.3f",$historiquearticle['Historiquearticle']['qte']);
    }
    if($historiquearticle['Historiquearticle']['mode']=="Sortie"){
        $qte_sor=$qte_sor+sprintf("%.3f",$historiquearticle['Historiquearticle']['qte']);
    }
    ...
    <?php if (!empty($historiquearticle['Historiquearticle']['utilisateur']))   { ?>     <?php echo $historiquearticle['Historiquearticle']['utilisateur']; ?> <?php }?>
           ...
           <?php }else{  ?>
            ...
            <td align="right"><?php echo @$historiquearticle['Historiquearticle']['remise']; ?></td>
            <td align="right"><?php echo @$historiquearticle['Historiquearticle']['ptot']; ?></td>
            <td align="right"><?php echo @$historiquearticle['Historiquearticle']['tva']; ?></td>
        </tr>
    <?php } ?>
        <tr>
            <td colspan="5"></td>
            <td align="right"><?php echo sprintf("%.3f",$qte_ent); ?></td>
            <td align="right"><?php echo sprintf("%.3f",$qte_sor); ?></td>
            <td colspan="4"></td>
        </tr>
        <tr>
            <td colspan="5"></td>
            <td colspan="2" align="center"><?php echo sprintf("%.3f",@$qte_final); ?></td>
            <td colspan="4"></td>
        </tr>
</table>
</div>
