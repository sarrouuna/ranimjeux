...
<div class="row">
    <div class="col-md-12">
        <a class="btn btn btn-danger" href="<?php echo $this->webroot;?>Reglementclients/addrpi"/> <i class="fa fa-plus-circle"></i> Ajouter </a>
    </div>
    </div>
    <?php } ?>
    <div class="row">
...
<div class="row">
                        <div class="col-md-12">
                            <div class="panel panel-default">
                                <div class="panel-heading">
                                    <h3 class="panel-title"><?php echo __('Règlements des impayés'); ?></h3>
                                </div>
                                <div class="panel-body">
	...
	<?php foreach ($reglementclients as $reglementclient):
             if($reglementclient['Reglementclient']['type']==2){?>
	<tr>
		...
		<td align="center">
			<?php echo $this->Html->link("<button class='btn btn-xs btn-success'><i class='fa fa-search'></i></button>", array('action' => 'viewrpi', $reglementclient['Reglementclient']['id']),array('escape' => false)); ?>
                        <?php if($edit==1){?><?php echo $this->Html->link("<button class='btn btn-xs btn-warning'><i class='fa fa-edit'></i></button>", array('action' => 'editrpi', $reglementclient['Reglementclient']['id']),array('escape' => false)); ?><?php } ?>
			<?php if($delete==1){?><?php echo $this->Form->postLink("<button class='btn btn-xs btn-danger'><i class='fa fa-trash-o'></i></button>", array('action' => 'delete', $reglementclient['Reglementclient']['id']),array('escape' => false,null), __('Veuillez vraiment supprimer cette enregistrement # %s?', $reglementclient['Reglementclient']['id'])); ?><?php } ?>
                        <?php if($imprimer==1){?><a onClick="flvFPW1(wr+'Reglementclients/imprimerviewrpi/'+<?php echo $reglementclient['Reglementclient']['id'];?>,'UPLOAD','width=800,height=1150,scrollbars=yes',0,2,2);return document.MM_returnValue" href="javascript:;" ><button class='btn btn-xs ls-blue-btn'><i class='fa fa-print'></i></button></a><?php } ?>
                </td>
	</tr>
<?php } endforeach; ?>
                          </tbody>
