<?php
App::uses('Facturesfournisseur', 'Model');

/**
 * Facturesfournisseur Test Case
 *
 */
class FacturesfournisseurTest extends CakeTestCase {

/**
 * Fixtures
 *
 * @var array
 */
	public $fixtures = array(
		'app.facturesfournisseur',
		'app.fournisseur',
		'app.famillefournisseur',
		'app.facturecharge',
		'app.facturefournisseur',
		'app.utilisateur',
		'app.fonction',
		'app.boncommande',
		'app.client',
		'app.timbre',
		'app.bonlivraison',
		'app.ligneclient',
		'app.ville',
		'app.pay',
		'app.devi',
		'app.lignedevi',
		'app.article',
		'app.famille',
		'app.marque',
		'app.inventaire',
		'app.depot',
		'app.lignelignecollection',
		'app.lignecollection',
		'app.collection',
		'app.lignecommande',
		'app.mouvementstock',
		'app.ligneproduction',
		'app.production',
		'app.personnel',
		'app.acompte',
		'app.moi',
		'app.annee',
		'app.paie',
		'app.ligneligneproduction',
		'app.stockdepot',
		'app.ligneinventaire',
		'app.reglement',
		'app.lignereglement',
		'app.piecereglement',
		'app.paiement',
		'app.piece',
		'app.banque',
		'app.lignebonlivraison',
		'app.ligneinventairefamille',
		'app.inventairefamille',
		'app.familletaille',
		'app.taille',
		'app.coutmp',
		'app.coutmpligne',
		'app.familie',
		'app.famillecomposant',
		'app.modele',
		'app.couleur',
		'app.type',
		'app.lignefacture',
		'app.facture',
		'app.transfereblfacture',
		'app.transferedeviscommande',
		'app.commande',
		'app.transferecommandebl',
		'app.bonsorty',
		'app.demande',
		'app.lignedemande',
		'app.matierepremiere',
		'app.unite',
		'app.family',
		'app.machine',
		'app.emplacement',
		'app.color',
		'app.typestock',
		'app.lbill',
		'app.bill',
		'app.bl',
		'app.bc',
		'app.lc',
		'app.lbl',
		'app.lot',
		'app.deposit',
		'app.entreemp',
		'app.livfournisseur',
		'app.lignelivfournisseur',
		'app.mpfournisseur',
		'app.entreepr',
		'app.lignepre',
		'app.lignempe',
		'app.inventory',
		'app.lineinventory',
		'app.retourstockmp',
		'app.retourstockmpligne',
		'app.sortiemp',
		'app.lignempsorti',
		'app.sortiepr',
		'app.ligneprsorti',
		'app.stockmatierepremiere',
		'app.lignelot',
		'app.lignesorty',
		'app.utilisateurmenu',
		'app.menu',
		'app.lien',
		'app.lignefacturefournisseur',
		'app.contact'
	);

/**
 * setUp method
 *
 * @return void
 */
	public function setUp() {
		parent::setUp();
		$this->Facturesfournisseur = ClassRegistry::init('Facturesfournisseur');
	}

/**
 * tearDown method
 *
 * @return void
 */
	public function tearDown() {
		unset($this->Facturesfournisseur);

		parent::tearDown();
	}

}
